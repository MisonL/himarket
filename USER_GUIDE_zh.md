...
+ OIDC / CAS 登录：支持第三方身份认证（如企业 SSO）
+ LDAP 身份验证：支持对接 Active Directory 或标准 LDAP 服务器，实现自动拨号 (Provisioning)。
+ **CAS 能力增强**：支持标准 CAS (Central Authentication Service) 协议集成，并覆盖以下关键能力：
    - **记住我 (Remember Me)**: 自动对接 CAS 长期授权票据，本地令牌 TTL 自动对齐 IdP 策略（默认极长有效）。
    - **属性动态同步**: 登录时自动从 Ticket 验证结果中同步 `displayName` 和 `email`。
    - **多值属性解析**: 支持 SAML 和 JSON 验证响应中的多值属性解析（例如多角色、多部门场景）。
