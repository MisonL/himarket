...
+ CAS 登录：支持标准 CAS (Central Authentication### 安全设置 (Security)

HiMarket 支持多种身份认证方式，以适应不同的企业级集成需求：

#### CAS 单点登录 (SSO)
*   **记住我 (Remember Me)**: 在 CAS 登录页勾选后，HiMarket 会遵循 CAS Server 的长效 Session 策略（最高可达 14 天）。
*   **属性动态同步**: 每次登录时会自动从 CAS 属性中同步用户的 `displayName` 和 `email`。
*   **单点注销 (SLO)**: 支持 SAML 和 CAS 格式的 Back-channel 注销请求。
*   **多值属性解析**: 完美支持 SAML 响应中的多值属性解析。

#### LDAP 身份验证
*   **企业集成**: 支持对接 Active Directory 或标准 LDAP 服务器。
*   **自动拨号 (Provisioning)**: 用户首次使用 LDAP 登录时，HiMarket 会根据 LDAP 属性自动创建开发者账号。
*   **配置项**: 支持配置 `serverUrl`, `baseDn`, `bindDn` 以及自定义的 `userSearchFilter`。
（例如多角色、多部门场景）。
    - **单点注销 (SLO)**: 支持 SAML LogoutRequest，确保 IdP 注销时本地会话及时失效。
