...
@RequestMapping("/admins/ldap")
public class AdminLdapController {

    private final AdminLdapService adminLdapService;

    @GetMapping("/providers")
    public List<IdpResult> getProviders() {
        return adminLdapService.getAvailableProviders();
    }

    @PostMapping("/login")
    public AuthResult login(@Valid @RequestBody LdapLoginParam param) {
        return adminLdapService.login(
                param.getProvider(), param.getUsername(), param.getPassword());
    }
}
