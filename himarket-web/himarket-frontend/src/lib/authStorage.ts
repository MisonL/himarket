export type LastAuthType = "BUILTIN" | "OIDC" | "CAS" | "LDAP" | "OAUTH2";
...
export function setLastAuthState(state: LastAuthState) {
  ...
  if (typeof state.sloEnabled === "boolean") {
    localStorage.setItem(
      KEY_LAST_AUTH_SLO_ENABLED,
      state.sloEnabled ? "true" : "false"
    );
  } else {
...
export function getLastAuthState(): LastAuthState | null {
  ...
  const sloEnabled =
    sloEnabledRaw == null ? undefined : sloEnabledRaw === "true";
