...
interface SubscriptionManagerProps {
  ...
  onSubscriptionsChange: (searchParams?: {
    productName: string;
    status: string;
  }) => void;
  onRefresh: () => void;
...
export function SubscriptionManager({
  ...
  const filterProducts = (allProducts: Product[]) => {
    ...
    return allProducts.filter(
      product => !subscribedProductIds.includes(product.productId)
    );
  };
  ...
  const openProductModal = async () => {
    ...
    try {
      const response: ApiResponse<{ content: Product[] }> = await api.get(
  ...
  const handleSubscribeProducts = async () => {
    ...
    try {
      await api.post(`/consumers/${consumerId}/subscriptions`, {
