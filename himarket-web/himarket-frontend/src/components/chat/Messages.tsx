          ...
          >
            {!isNewChat && expandedContent && (
              <div
                ...
                className="bottom-mask flex justify-center items-end cursor-pointer absolute -bottom-px h-14 w-full "
                style={{
                  background:
                ...
                {question.messageChunks.map(chunk => {
                  ...
                  if (chunk.type === "tool_call" && chunk.toolCall) {
                    ...
                    const toolResultChunk = question.messageChunks?.find(
                      c =>
                        c.type === "tool_result" &&
                        c.toolResult?.id === chunk.toolCall?.id
                    );
                    return (
                      <McpToolCallItem
                        key={chunk.id}
          ...
          {/* 统计信息和功能按钮 - 只在有内容或错误时显示 */}
          {
            <div className="flex items-center justify-between mt-2 px-1">
              ...
              <div className="flex items-center gap-1">
                <button
                  onClick={() =>
                    handleCopy(activeAnswer?.content || "", question.id)
                  }
                  className={`
                ...
                {question.answers?.length > 1 && (
                  <div className="flex items-center gap-1 mr-2 px-2 py-1 rounded-md">
                    <button
                      onClick={() =>
                        onChangeVersion?.(conversation.id, question.id, "prev")
                      }
                    ...
                    <button
                      onClick={() =>
                        onChangeVersion?.(conversation.id, question.id, "next")
                      }
                      disabled={
                        question.activeAnswerIndex ===
                        question.answers.length - 1
                      }
                      className={`
                                p-1 rounded transition-colors duration-200
