...
export function ApiProductLinkApi({
  ...
  const [selectedNacos, setSelectedNacos] = useState<NacosInstance | null>(
    null
  );
  ...
  const [selectedAgentDomainIndex, setSelectedAgentDomainIndex] =
    useState<number>(0);
    ...
    if (apiProduct.type === "MCP_SERVER" && apiProduct.mcpConfig) {
      ...
      if (linkedService) {
        ...
        if (
          ...
          mcpServerName =
            linkedService.apigRefConfig.mcpServerName || apiProduct.name;
        } else if (
          linkedService.sourceType === "GATEWAY" &&
          linkedService.higressRefConfig
        ) {
          mcpServerName =
            linkedService.higressRefConfig.mcpServerName || apiProduct.name;
        } else if (
          linkedService.sourceType === "GATEWAY" &&
          ...
          if ("modelApiName" in linkedService.adpAIGatewayRefConfig) {
            mcpServerName =
              linkedService.adpAIGatewayRefConfig.modelApiName ||
              apiProduct.name;
          } else {
            // APIGAIMCPItem 类型
            mcpServerName =
              linkedService.adpAIGatewayRefConfig.mcpServerName ||
              apiProduct.name;
          }
        } else if (
          linkedService.sourceType === "GATEWAY" &&
          ...
          if ("modelApiName" in linkedService.apsaraGatewayRefConfig) {
            mcpServerName =
              linkedService.apsaraGatewayRefConfig.modelApiName ||
              apiProduct.name;
          } else {
            mcpServerName =
              linkedService.apsaraGatewayRefConfig.mcpServerName ||
              apiProduct.name;
          }
        } else if (
          ...
          mcpServerName =
            linkedService.nacosRefConfig.mcpServerName || apiProduct.name;
        }
      }
    ...
    try {
      ...
      if (apiProduct.type === "REST_API") {
        ...
        result = res.data?.content?.filter?.(
          (item: Gateway) => item.gatewayType === "APIG_API"
        );
      } else if (apiProduct.type === "AGENT_API") {
        // Agent API 只支持 APIG_AI 网关
        result = res.data?.content?.filter?.(
          (item: Gateway) => item.gatewayType === "APIG_AI"
        );
  ...
  const handleNacosChange = async (nacosId: string) => {
    ...
    // 获取命名空间列表
    try {
      const nsRes = await nacosApi.getNamespaces(nacosId, {
        page: 1,
        size: 1000,
      });
    ...
    form.validateFields().then((values: any) => {
      ...
      const selectedApi = apiList.find((item: any) => {
        if ("apiId" in item) {
          ...
          "modelRouteName" in item &&
          item.fromGatewayType === "HIGRESS"
        ) {
      ...
      const newService: LinkedService = {
        ...
        adpAIGatewayRefConfig:
          ...
          selectedApi.fromGatewayType === "ADP_AI_GATEWAY"
            ? apiProduct.type === "MODEL_API"
              ? ({
                  modelApiId: (selectedApi as any).modelApiId,
                  modelApiName: (selectedApi as any).modelApiName,
                  fromGatewayType: "ADP_AI_GATEWAY" as const,
      ...
      apiProductApi
        ...
        .then(async () => {
          ...
          // 重新获取关联信息并更新
          try {
            const res = await apiProductApi.getApiProductRef(
              apiProduct.productId
            );
    ...
    if (apiProduct.type === "REST_API") {
      ...
      if (
        ...
        if ("modelApiName" in linkedService.adpAIGatewayRefConfig) {
          // 专有云AI网关上的Model API
          apiName =
            linkedService.adpAIGatewayRefConfig.modelApiName || "未命名";
          sourceInfo = "专有云AI网关";
          gatewayInfo = linkedService.gatewayId || "未知";
        } else {
          // 专有云AI网关上的MCP Server
          apiName =
            linkedService.adpAIGatewayRefConfig.mcpServerName || "未命名";
          sourceInfo = "专有云AI网关";
          gatewayInfo = linkedService.gatewayId || "未知";
        }
      } else if (
        ...
        if ("mcpServerName" in linkedService.apsaraGatewayRefConfig) {
          apiName =
            linkedService.apsaraGatewayRefConfig.mcpServerName || "未命名";
        } else {
          apiName =
            linkedService.apsaraGatewayRefConfig.modelApiName || "未命名";
        }
