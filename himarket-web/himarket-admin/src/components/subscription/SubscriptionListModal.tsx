...
export function SubscriptionListModal({
  ...
  const handleApproveSubscription = async (subscription: Subscription) => {
    setActionLoading(
      `${subscription.consumerId}-${subscription.productId}-approve`
    );
    try {
      await portalApi.approveSubscription(
  ...
  const handleDeleteSubscription = async (subscription: Subscription) => {
    setActionLoading(
      `${subscription.consumerId}-${subscription.productId}-delete`
    );
    try {
      await portalApi.deleteSubscription(
        subscription.consumerId,
        subscription.productId
      );
        ...
        if (record.status === "PENDING") {
          ...
          return (
            <Popconfirm
              ...
              cancelText="取消"
            >
              <Button type="default" size="small" danger loading={isDeleting}>
                删除订阅
              </Button>
            </Popconfirm>
          );
