...
export function PortalSettings({ portal, onRefresh }: PortalSettingsProps) {
  ...
  const handleSaveThirdPartyAuth = async (configs: ThirdPartyAuthConfig[]) => {
    try {
      ...
      const oidcConfigs = configs
        ...
        .map(config => {
          const { type, ...oidcConfig } = config as OidcConfig & {
            type: AuthenticationType.OIDC;
          };
          return oidcConfig;
        });
      ...
      const casConfigs = configs
        ...
        .map(config => {
          const { type, ...casConfig } = config as CasConfig & {
            type: AuthenticationType.CAS;
          };
          return casConfig;
        });
      ...
      const oauth2Configs = configs
        ...
        .map(config => {
          const { type, ...oauth2Config } = config as OAuth2Config & {
            type: AuthenticationType.OAUTH2;
          };
          return oauth2Config;
        });
      ...
      const ldapConfigs = configs
        ...
        .map(config => {
          const { type, ...ldapConfig } = config as LdapConfig & {
            type: AuthenticationType.LDAP;
          };
          return ldapConfig;
        });
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            >
              <Switch
                onChange={checked =>
                  handleSettingUpdate("builtinAuthEnabled", checked)
                }
              />
            </Form.Item>
            <Form.Item
            ...
            >
              <Switch
                onChange={checked =>
                  handleSettingUpdate("autoApproveDevelopers", checked)
                }
              />
            </Form.Item>
            <Form.Item
            ...
            >
              <Switch
                onChange={checked =>
                  handleSettingUpdate("autoApproveSubscriptions", checked)
                }
              />
            </Form.Item>
          </div>
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      >
        <Card>
          <Tabs items={tabItems} defaultActiveKey="auth" type="card" />
        </Card>
      </Form>
