...
export function PublicKeyManager({
  provider,
  publicKeys,
  onSave,
}: PublicKeyManagerProps) {
  const [form] = Form.useForm<PublicKeyFormData>();
  const [modalVisible, setModalVisible] = useState(false);
  const [editingIndex, setEditingIndex] = useState<number | null>(null);
  const [localPublicKeys, setLocalPublicKeys] =
    useState<PublicKeyConfig[]>(publicKeys);
  const [selectedFormat, setSelectedFormat] = useState<PublicKeyFormat>(
    PublicKeyFormat.PEM
  );
      ...
      render: (record: PublicKeyConfig) => (
        <span className="font-mono text-xs text-gray-600">
          {record.format === PublicKeyFormat.PEM
            ...
            : JSON.stringify(JSON.parse(record.value || "{}")).substring(
                0,
                50
              ) + "..."}
        </span>
