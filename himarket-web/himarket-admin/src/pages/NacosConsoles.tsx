...
import NacosTypeSelector, {
  NacosImportType,
} from "@/components/console/NacosTypeSelector";
    ...
    },
    {
      ...
      render: (val: any, record: NacosInstance) => {
        const t =
          val ??
          record.createAt ??
          (record as any).createTime ??
          (record as any).gmtCreate;
      ...
      >
        <Form form={form} layout="vertical" initialValues={{}}>
          ...
          <Form.Item
            name="serverUrl"
            label="服务器地址"
            rules={[{ required: true, message: "请选择或输入服务器地址" }]}
          >
          ...
          <Form.Item name="description" label="描述">
            <Input.TextArea rows={3} placeholder="请输入实例描述（可选）" />
          </Form.Item>
        </Form>
      </Modal>
