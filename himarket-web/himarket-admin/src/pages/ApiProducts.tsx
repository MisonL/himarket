...
export default function ApiProducts() {
  ...
  const fetchApiProducts = useCallback(
    (page = 1, size = 12, tab = activeTab, name = nameFilter) => {
      setLoading(true);
      apiProductApi
        .getApiProducts(buildParams(page, size, tab, name))
        .then((res: any) => {
              ...
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {apiProducts.map((product) => (
                  <ProductCard
                    ...
                    handleRefresh={() => fetchApiProducts(pagination.current, pagination.pageSize, activeTab, nameFilter)}
                    onEdit={handleEdit}
                  />
                ))}
              </div>
