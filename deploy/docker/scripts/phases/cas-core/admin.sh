...
verify_admin_cas_protocol_flows() {
  ...
  verify_cas_browser_flow \
    "admin cas" \
    "${HIMARKET_BASE_URL}/admins/cas/authorize?provider=cas" \
    "${HIMARKET_BASE_URL}/admins/cas/exchange" \
    "${HIMARKET_BASE_URL}/admins" \
    "admin" \
    "admin" \
    "none" \
  ...
  curl -fsS -X POST "${HIMARKET_BASE_URL}/admins/cas/proxy-ticket" \
  ...
  admin_proxy_reject="$(curl_json POST "${HIMARKET_BASE_URL}/admins/cas/proxy-ticket" '{"provider":"cas","targetService":"http://localhost:5173/forbidden"}' "Authorization: Bearer ${admin_cas_token}")"
  ...
  expect_auth_rejected "admin cas revoked token" "${HIMARKET_BASE_URL}/admins" "${admin_cas_token}"

  log "admin cas saml1 authorize"
  verify_cas_browser_flow \
    "admin cas saml1" \
    "${HIMARKET_BASE_URL}/admins/cas/authorize?provider=cas-saml1" \
    "${HIMARKET_BASE_URL}/admins/cas/exchange" \
    "${HIMARKET_BASE_URL}/admins" \
    "admin" \
    "admin" \
  ...
  verify_cas_browser_flow \
    "admin cas1" \
    "${HIMARKET_BASE_URL}/admins/cas/authorize?provider=cas1" \
    "${HIMARKET_BASE_URL}/admins/cas/exchange" \
    "${HIMARKET_BASE_URL}/admins" \
    "admin" \
    "admin" \
  ...
  verify_cas_browser_flow \
    "admin cas2" \
    "${HIMARKET_BASE_URL}/admins/cas/authorize?provider=cas2" \
    "${HIMARKET_BASE_URL}/admins/cas/exchange" \
    "${HIMARKET_BASE_URL}/admins" \
    "admin" \
    "admin" \
